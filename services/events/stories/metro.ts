...
export const METRO_NODES: Record<string, StoryNode> = {
    ...
    'metro_1_entrance': {
        ...
        next: [
            { id: 'metro_2_tunnel', weight: 0.5 }, // 선로 (기존)
            { id: 'metro_2_utility', weight: 0.3 }, // 관리실 (신규)
            { id: 'metro_2_flooded', weight: 0.2 }  // 침수 구역 (신규)
        ],
    ...
    'metro_3_power_fail': {
        ...
        next: [{ id: 'metro_5_ambush', weight: 1.0 }], // 바로 전투로
    ...
    'metro_2_flooded': {
        id: 'metro_2_flooded',
        text: "🌊 선로가 물에 잠겨 있습니다. 뗏목을 만들어 건너가면 좀비들을 피할 수 있지만, 물속에 무엇이 있을지 모릅니다.",
        next: [
            { id: 'metro_3_swim_success', weight: 0.6 },
            { id: 'metro_3_swim_fail', weight: 0.4 }
        ]
    },
    'metro_3_swim_success': {
        id: 'metro_3_swim_success',
        text: "🛶 쓰레기로 만든 뗏목으로 조용히 물 위를 건넜습니다. 체력을 아끼고 안전하게 이동했습니다.",
        next: [{ id: 'metro_7_end', weight: 1.0 }], // 바로 탈출
        effect: { target: 'ALL', fatigue: -10 }
    },
    'metro_3_swim_fail': {
        id: 'metro_3_swim_fail',
        text: "🐊 물속에서 부푼 시체들이 떠올라 뗏목을 뒤집었습니다! 썩은 물을 마시고 허우적대며 간신히 기어 나왔습니다.",
        next: [{ id: 'metro_4_nest', weight: 1.0 }],
        effect: { target: 'RANDOM_HALF', infection: 20, sanity: -15 }
    },
